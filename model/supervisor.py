...
class Supervisor():
    def __init__(self, train_loader, val_loader, model,
        ...
        logdir = '../output/_testlog'
        if "logdir" in params:
            logdir = params['logdir']
        debug_log = True
        if "debug_log" in params:
            debug_log = params['debug_log']
        debug_freq = 250
        if "debug_freq" in params:
            debug_freq = params['debug_freq']
        display_freq = 50
        if "display_freq" in params:
            display_freq = params['display_freq']
        checkpoint_freq = 250
        if "checkpoint_freq" in params:
            checkpoint_freq = params['checkpoint_freq']
        load_path = None
        if "load_path" in params:
            load_path = params['load_path']
        zero_counters = False
        if "zero_counters" in params:
            zero_counters = params['zero_counters']
