...
from environment.objects import CoordinateVis, YCBObject
...
class TabletopEnv(SingleArmEnv):
    ...
    def _load_model(self):
        ...
        self.robots[0].gripper.current_action = [1.0, -1.0]
    ...
    def _setup_observables(self):
        ...
        def gripper_action(obs_cache):
            return self.robots[0].gripper.current_action[1]
        ...
        def gripper_closed(obs_cache):
            if self.grasped_obj is not None:
                ...
                return (self.robots[0].gripper.current_action[1] > 0.9)
    ...
    def get_robot_configuration(self):
        ...
        gripper['right_gripper'] = (
            self.sim.data.get_body_xpos('gripper0_right_gripper'),
            self.sim.data.get_body_xquat('gripper0_right_gripper')
        )
        gripper['leftfinger'] = (
            self.sim.data.get_body_xpos('gripper0_leftfinger'),
            self.sim.data.get_body_xquat('gripper0_leftfinger')
        )
        gripper['rightfinger'] = (
            self.sim.data.get_body_xpos('gripper0_rightfinger'),
            self.sim.data.get_body_xquat('gripper0_rightfinger')
        )
    ...
    def apply_external_poses(self, poses):
        for i, obj in enumerate(self.objects):
            ...
            self.sim.data.set_joint_qpos(
                ...
                np.concatenate(
                    [
                        poses[i][0] + np.array([0.0, 0.0, 0.005]),
